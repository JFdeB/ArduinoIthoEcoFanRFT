...
bool CC1101::sendData(CC1101Packet *packet)
{
  ...
  length = (packet->length <= CC1101_DATA_LEN ? packet->length : CC1101_DATA_LEN); // CC1101_DATA_LEN = 61
  ...
  if (packet->length > CC1101_DATA_LEN)
  {
    ...
    while (index < packet->length)
    {
      ...
      /* The above statement was replaced by the statement below for the following reason:
       *  Due to a bug un the CC1101 hardware (see CC1101 erata) a single read of some registers can give incorrect results. The solution is to read these registers twice 
       *  (readRegisterWithSyncProblem()) and if the values are the same the result is considered reliable. However, this approach fails for reading the number of bytes 
       *  in the TX_FIFO during transmission, since this number is continuously updated, also in between the two consequtive reads. Therefore the above statement only reads
       *  the same value twice when the transmisson is completed and the TX_FIFO is empty. But that is too late to write the remaining bytes of the buffer to the TX_FIFO and
       *  the subroutine hangs on a TX_FIFO_BUFFER_UNDERFLOW.
       *  The statement below reads number of bytes in the TX_FIFO as a median of 3 values (readRegisterMedian3()). (High probability that this is correct).  JFdB
       */
